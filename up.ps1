...
dotnet sitecore login --cm https://cm.jss_1.localhost/ --auth https://id.jss_1.localhost/ --allow-write true
if ($LASTEXITCODE -ne 0) {
...
Invoke-RestMethod "https://cm.jss_1.localhost/sitecore/admin/PopulateManagedSchema.aspx?indexes=all" -Headers @{Authorization = "Bearer $token"} -UseBasicParsing | Out-Null
...
Start-Process https://cm.jss_1.localhost/sitecore/
Start-Process https://www.jss_1.localhost/
